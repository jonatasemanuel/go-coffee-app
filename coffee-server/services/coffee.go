...
type Coffee struct {
	...
	GrindUnit int16     `json:"grind_uint"`
...
func (c *Coffee) GetCoffeeById(id string) (*Coffee, error) {
	...
	query := `SELECT 
		...
		grind_unit, 
		created_at, 
		updated_at FROM coffees WHERE id = $1
	`
	var coffee Coffee
...
func (c *Coffee) CreateCoffee(coffee Coffee) (*Coffee, error) {
	...
	query := `INSERT INTO coffees 
		(name, image, region, roast, price, grind_unit, created_at, updated_at)
